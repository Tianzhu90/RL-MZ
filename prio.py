    ...
    def _build_net(self):
        def build_layers(s, c_names, n_l1, w_initializer, b_initializer, trainable):
            ...
            if self.dueling:
                ...
                with tf.variable_scope('Value'):
                    w4 = tf.get_variable('w4', [n_l1, n_l1/2], initializer=w_initializer, collections=c_names)
                    b4 = tf.get_variable('b4', [1, n_l1/2], initializer=b_initializer, collections=c_names)
                    V = tf.nn.relu(tf.matmul(l2, w4) + b4)

                    w5 = tf.get_variable('w5', [n_l1/2, 1], initializer=w_initializer, collections=c_names)
                    b5 = tf.get_variable('b5', [1, 1], initializer=b_initializer, collections=c_names)
                    self.V = tf.matmul(V, w5) + b5


                with tf.variable_scope('Advantage'):
                    w4 = tf.get_variable('w4', [n_l1, n_l1/2], initializer=w_initializer, collections=c_names)
                    b4 = tf.get_variable('b4', [1, n_l1/2], initializer=b_initializer, collections=c_names)
                    A = tf.nn.relu(tf.matmul(l2, w4) + b4)

                    w5 = tf.get_variable('w5', [n_l1/2, self.n_actions], initializer=w_initializer, collections=c_names)
                    b5 = tf.get_variable('b5', [1, self.n_actions], initializer=b_initializer, collections=c_names)
                    self.A = tf.matmul(A, w5) + b5
        ...
        with tf.variable_scope('loss'):
            if self.prioritized:
                ...
                difference = tf.abs(self.q_eval - self.q_target)
                quadratic_part = tf.clip_by_value(difference, 0.0, 1.0)
                linear_part = difference - quadratic_part

                # self.loss = tf.reduce_mean(self.ISWeights * tf.squared_difference(self.q_target, self.q_eval))
                self.loss = tf.reduce_mean(self.ISWeights * (0.5* tf.square(quadratic_part)+linear_part))
            else:
